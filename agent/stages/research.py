...
def run_research(out_dir: Path, queries: list[str], max_arxiv: int, cycle_id: str, day_id: str) -> Path:
    ...
    lines += [
        ...
        "",
        "1. **Keep gold_rank_w50 (0.518) frozen** and fit ranks on train-report weak labels (`weak_rank_calibrate`). gold_rank_lam2 scored 0.515 and is falsified; blend weights have plateaued.",
        "2. Keep **gold_rank_w50** as the fallback notebook if weak-label ranks do not beat 0.518. Do not resubmit gold_rank_lam2 (0.515), gold_rank_w40 (tied 0.518), gold_rank_w70 (0.516), or 0.50/0.50 as cycle 0.",
        ...
        "",
        "Implication for this cycle: visual AUCs of 0.8–0.9 and grouped-fold metadata ~0.60 remain medium-run targets. Gold-only ranking has stalled: w50=0.518, w40=0.518, lam2=0.515. The only *currently executable* ranking lever that does not invent DICOM-header paths is fitting the same 7-d/13-d series-flag heads on train-report weak labels (n≈4407) and using gold only for calibration — still no test reports.",
        "",
        "### arXiv query hits",
        "",
    ]
