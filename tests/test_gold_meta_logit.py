...
def test_cycle0_plan_selects_weak_rank_confident(tmp_path):
    ...
    dummy.write_text("x")
    md = run_plan(tmp_path, dummy, dummy, dummy, cycle_id="00_test", day_id="2026-09-23", cycle_num=0)
    assert "weak_rank_confident" in md.read_text()
    sidecar = json.loads((tmp_path / "2026-09-23_cycle00_test_plan.json").read_text())
